function  [v,f] = SDK_simultaneoussubdividesurface(obj,maxMaxEdgeLength,maxVertices,maxConnections,buldgeFactor)
warning('off','all')
%nested functions:
...
while maxLength > maxMaxEdgeLength && size(triangles.Points,1)<=maxVertices && iterations < maxIterations
    disp(iterations)
    
    edgeLengthHeap = computeEdgeLengths();
    ComputeMaxEdgeLength(edgeLengthHeap);
    newVertices = [];
    template = triangles;
    ...
    for iEdge = 1:size(template.edges,1)
        ...
        if edgeLengthHeap(iEdge) <= maxMaxEdgeLength;
            continue;
        end
