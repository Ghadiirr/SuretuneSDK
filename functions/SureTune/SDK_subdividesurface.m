function  [v,f] = SDK_subdividesurface(obj,maxMaxEdgeLength,maxVertices,maxConnections,buldgeFactor)
warning('on','all')
% %nested functions:
...
while  size(triangles.Points,1)<=maxVertices
    waitbar(size(triangles.Points,1)/maxVertice)
    
    % compute edge lengths
    edgeLengthHeap = computeEdgeLengths();
    
    % find the longest edge
    ComputeMaxEdgeLength(edgeLengthHeap) %returns: maxLength and maxLengthEdge
    ...
    if maxLength > prevLength
        error('Edges seem to be getting bigger!')
    end
    
    % update prevLength to current maxLength
    prevLength = maxLength;
    
    %split edge into two
    newPointIdx = splitEdgeIntoTwo(maxLengthEdge);
    variableVertices(end+1) = newPointIdx;
