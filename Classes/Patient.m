classdef Patient < SessionComponent
    ...
    methods
        function obj = Patient(path,session)%,name,patientID,dateOfBirth,gender)
            ...
            obj.name = trytoget(['session.sessionData.',session.ver,'.Session.patient.Patient.name.Attributes.value']);
            obj.patientID = trytoget(['session.sessionData.',session.ver,'.Session.patient.Patient.patientID.Attributes.value']);
            obj.dateOfBirth = trytoget(['session.sessionData.',session.ver,'.Session.patient.Patient.dateOfBirth.Attributes.value']);
            obj.gender = trytoget(['session.sessionData.',session.ver,'.Session.patient.Patient.gender.Enum.Attributes.value']);
